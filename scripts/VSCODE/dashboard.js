...
async function loadNews(reset = false) {
  ...
  const newsRaw = json.news || json; // תמיכה בשני פורמטים

  // מיון מהתאריך החדש לישן (עמיד גם אם יש ערכים חסרים)
  const newsSorted = [...newsRaw].sort((a, b) => {
  const da = a?.Date ? new Date(a.Date) : 0;
  const db = b?.Date ? new Date(b.Date) : 0;
  return db - da; // יורד: חדש→ישן
});

const list = document.getElementById('newsList');
const slice = newsSorted.slice(newsOffset, newsOffset + PAGE_SIZE);
...
// טעינת הגרפים של RSI, MACD ונפח – מתוך market_data.json
async function loadMetricsCharts() {
  try {
    const raw = await fetch('data/market_data.json'); // ← הקובץ הנכון
    const data = await raw.json();

    // מוודא מיון כרונולוגי ולוקח את 5 ימי המסחר האחרונים
    const sorted = [...data].sort((a, b) => new Date(a.Date) - new Date(b.Date));
    const last5 = sorted.slice(-5);

    // שדות לפי ה-JSON (אותיות גדולות)
    const dates  = last5.map(r => r.Date);
    const rsi    = last5.map(r => r.RSI ?? null);
    const macd   = last5.map(r => r.MACD ?? null);
    const volume = last5.map(r => r.Volume ?? null); // ← לא volatility

    // חישוב ספי מינימום/מקסימום
    const maxVol = Math.max(...volume.filter(v => v != null));
    const minVol = Math.min(...volume.filter(v => v != null));

    // פונקציה שממפה נפח לצבע: 0=אדום, 1=ירוק
    const getColor = (v) => {
      if (v == null || !isFinite(v)) return '#888';
      const t = (v - minVol) / Math.max(1, (maxVol - minVol)); // נורמליזציה בטוחה
      const hue = 120 * t; // 0=אדום, 120=ירוק
      return `hsl(${hue}, 85%, 50%)`;
    };
    // הגדרת tooltip שלא ייפול על null + חיבור spanGaps כדי לדלג על פערים
    const safeLabel = (prefix) => (ctx) =>
      ctx.parsed.y == null ? `${prefix}: N/A` : `${prefix}: ${ctx.parsed.y.toFixed(2)}`;

    new Chart(document.getElementById('rsiChart'), {
      type: 'line',
      data: { labels: dates, datasets: [{ label: 'RSI', data: rsi,
        borderColor: '#FFD460', backgroundColor: 'rgba(255,212,96,0.2)', fill: true, tension: 0.4, pointRadius: 5 }] },
      options: {
        responsive: true, spanGaps: true,
        plugins: { legend: { display: false }, tooltip: { enabled: true, callbacks: { label: safeLabel('RSI') } } },
        interaction: { mode: 'index', intersect: false },
        scales: { x: { ticks: { color: '#ccc' } }, y: { ticks: { color: '#ccc' }, suggestedMin: 0, suggestedMax: 100 } }
      }
    });

    new Chart(document.getElementById('macdChart'), {
      type: 'line',
      data: { labels: dates, datasets: [{ label: 'MACD', data: macd,
        borderColor: '#3A99FF', backgroundColor: 'rgba(58,153,255,0.2)', fill: true, tension: 0.4, pointRadius: 5 }] },
      options: {
        responsive: true, spanGaps: true,
        plugins: { legend: { display: false }, tooltip: { enabled: true, callbacks: { label: safeLabel('MACD') } } },
        interaction: { mode: 'index', intersect: false },
  ...
  new Chart(document.getElementById('volumeChart'), {
    ...
    data: {
      ...
      datasets: [{
        label: 'נפח מסחר',
        data: volume,
        backgroundColor: volume.map(getColor),
        borderColor: volume.map(getColor),
        borderWidth: 1,
        borderRadius: 6
    }]
  },
  options: {
    ...
    plugins: {
      ...
      tooltip: {
        enabled: true,
        callbacks: {
          label: (ctx) => `Volume: ${ctx.parsed.y == null ? 'N/A' : ctx.parsed.y.toLocaleString()}`
        }
      }
    },
    scales: {
    ...
    console.error('Metrics charts error:', err);
  }
}
